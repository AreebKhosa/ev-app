...
import { Plus, Edit3, Trash2, Search, Gauge, BatteryCharging, Bike } from "lucide-react";
...
export function AdminProducts({
  ...
  return (
    <div className="space-y-6">
      ...
      {filtered.length === 0 ? (
        <div className="p-12 text-center rounded-3xl backdrop-blur-2xl bg-white/70 dark:bg-[#121316]/80 border border-black/10 dark:border-white/10 space-y-4">
          ...
          <h3 className="text-base font-bold text-neutral-900 dark:text-white">
            {products.length === 0 ? "No Fleet Models in Database" : "No Models Match Your Search"}
          </h3>
          <p className="text-xs text-neutral-500 max-w-sm mx-auto">
            {products.length === 0
              ? "Deploy your first electric vehicle model with specifications, category, and photo."
              : "Try adjusting your search keywords."}
          </p>
          {products.length === 0 && (
            <button
              onClick={onAddProduct}
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#D4FF00] text-black font-bold text-xs shadow-md hover:scale-105 active:scale-95 transition-all cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              <span>Deploy First Fleet Model</span>
            </button>
          )}
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((product) => (
            <div
              key={product.id}
              className="rounded-3xl backdrop-blur-2xl bg-white/70 dark:bg-[#121316]/80 border border-black/10 dark:border-white/10 p-5 shadow-sm space-y-4 flex flex-col justify-between hover:border-[#D4FF00]/40 transition-colors"
            >
